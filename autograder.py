...
def check_graph_basic(tracker):
    ...
    assert graph.get_nodes() == [v1, v2, loss], \
        "Not all nodes are present after adding a node loss: {}, inputs {}, gradient {}, outputs{}".format(loss, graph.get_inputs(loss), graph.get_gradient(loss), graph.get_output(loss))
