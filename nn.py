...
class Graph(object):
    ...
    def __init__(self, variables):
        ...
        #print variables
        for v in variables:
            #print "%s's parents are %s" % (v, v.get_parents())
            self.add(v)
        #print self.SoftmaxLoss(variables[0],variables[1])
    ...
    def get_inputs(self, node):
        ...
        for p in node.get_parents():
            #print p
            stuff.append(self.get_output(p))
        return stuff
    ...
    def add(self, node):
        ...
        #print "adding %s with output%s" % (node, output)
        #print output
        self.outputs[node] = output
        numEntries = np.prod(output.shape)
        if output.shape != ():
            self.gradients[node] = np.zeros(numEntries).reshape(output.shape)
        "*** YOUR CODE HERE ***"
    ...
    def backprop(self):
        ...
        for v in reversed(nodes):
            grad = v.backward(self.get_inputs(v), self.gradients[v])
            count = 0
            for i in v.get_parents():
                self.gradients[i] += grad[count]
                count += 1
            #print "%s inputs are %s" % (v, self.get_inputs(v))
            #print self.get_gradient(v)
            #print self.get_output(v)
            #print grad
            #self.gradients[v] = grad + self.gradients[v]
    ...
    def step(self, step_size):
        ...
        for v in self.variables:
            gradient = self.gradients[v]
            v.data -= gradient * step_size
...
class MatrixMultiply(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        one = np.dot(gradient, inputs[1].T)
        two = np.dot(inputs[0].T, gradient)
...
class MatrixVectorAdd(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        grad1 = np.copy(gradient)
        grad1.reshape(inputs[0].shape)
        return [grad1, np.sum(gradient, axis = 0)]
        "*** YOUR CODE HERE ***"
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        x = inputs[0]
        numEntries = np.prod(x.shape)
        zeros = np.zeros(numEntries).reshape(x.shape)
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        ...
        b = inputs[1]
        c = a - b
        return np.mean(0.5 * np.power(c, 2))
        "*** YOUR CODE HERE ***"
    ...
    def backward(inputs, gradient):
        ...
        b = inputs[1]
        c = a - b
        #print np.mean(c)/gradient
        c *= gradient/inputs[0].size
        return [c, -1*c]
        "*** YOUR CODE HERE ***"
